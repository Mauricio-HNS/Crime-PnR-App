...
class _CrimeAlertsScreenState extends State<CrimeAlertsScreen> {
  ...
  Future getAlertList()async{
    ...
    await reportRef.onValue.listen((event) async {
      for (final child in event.snapshot.children) {
        ...
        var ln = double.parse(data['longitude']);
          ...
          infoWindow: InfoWindow(title: data["type"], snippet: data["date"])));
  ...
  @override
  void initState() {
    initialCameraPosition = CameraPosition(target: LatLng(lng, lat), zoom: 16.0);
    super.initState();
    getCurrentLocation();
  }
            ...
            child: ElevatedButton(
                ...
                child: Row(
                  children: [
                    ...
                    Text("Enter Area, City or State", style: TextStyle(color: Colors.grey.shade600, fontSize: 16),)
                  ],
                )),
          ),
          Align(
            ...
            child: Padding(
              padding: const EdgeInsets.only(bottom: 10.0, left: 10, right: 100),
              child: Container(
                color: Colors.white,
  ...
  Future<void> _handlePressButton() async {
    Prediction? p = await PlacesAutocomplete.show(
        ...
        components: [Component(Component.country,"pk"),
         Component(Component.country,"usa"),Component(Component.country,"my")]);
  ...
  Future<void> displayPrediction(Prediction p,
      ScaffoldState? currentState) async {
